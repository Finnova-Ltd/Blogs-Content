...
window.switchHomeSubToggle = function(subKey) {
  ...
  /* ==========================================================================
     VERTICAL SLIDING GOOGLE REVIEWS CAROUSEL (15s CYCLE)
     (Auto-mounts on all Article, Calculator, Loan and Location Sidebars)
     ========================================================================== */
  function initSidebarReviewsCarousels() {
    // Find all sidebar review containers across articles, calculators, loans, and locations
    const containers = document.querySelectorAll(
      '.sidebar-reviews-carousel, .google-reviews-sidebar, [data-google-reviews-sidebar], #calculator-google-reviews, .sidebar-accordion-widget.google-reviews-widget'
    );

    if (!containers || containers.length === 0) return;

    const reviews = (typeof VERIFIED_GOOGLE_REVIEWS !== 'undefined' && Array.isArray(VERIFIED_GOOGLE_REVIEWS)) 
      ? VERIFIED_GOOGLE_REVIEWS 
      : [
          { author_name: 'Jaspreet Sidhu', rating: 5, text: 'EZ Mortgage Broker has been helping me for all my financial needs like first home buyer loan and refinance. Every time they helped me a lot, they are very professional, honest, reliable, they know their job well and treat you like family.', relative_time_description: '3 months ago' },
          { author_name: 'Navtej Singh', rating: 5, text: 'This is my second time working with EZ Mortgage Broker to purchase my dream home. The entire journey was smooth and stress-free from start to finish. They guided me through every document and step with great care.', relative_time_description: '10 months ago' },
          { author_name: 'N Cassim', rating: 5, text: 'From the start EZ Mortgage Broker guided us step by step and helped us in our financial matter. They took stock of our financial situation and gave us great pertinent advice. Highly recommend!', relative_time_description: '4 months ago' },
          { author_name: 'Emily', rating: 5, text: 'EZ Mortgage Broker has been an outstanding support throughout my entire loan process and that of my family. Their professionalism, responsiveness, and dedication are unparalleled.', relative_time_description: '1 year ago' },
          { author_name: 'Justin Gray', rating: 5, text: 'EZ Mortgage Broker has been fantastic throughout. Secured an exceptional rate with a great repayment structure and kept us updated on settlement daily.', relative_time_description: '7 months ago' },
          { author_name: 'Rod Wonnacott', rating: 5, text: 'I found EZ Mortgage Broker to be very professional, friendly, supportive and efficient in securing finance for my business. They consulted and informed excellently.', relative_time_description: 'Recently' },
          { author_name: 'Nikki Patel', rating: 5, text: 'I would like to thank EZ Mortgage Broker for their excellent service throughout my loan process. Everything was handled smoothly, clearly, and with great professionalism.', relative_time_description: 'Recently' },
          { author_name: 'Ajay Joshi', rating: 5, text: 'Excellent service and deep knowledge of lender policies. Got our home loan pre-approved within 48 hours without any hassle. Highly recommended!', relative_time_description: '3 months ago' }
        ];

    containers.forEach(function (container) {
      // Avoid initializing on homepage main marquee
      if (container.id === 'google-reviews') return;

      // Build standard vertical carousel DOM
      container.innerHTML = `
        <div class="sidebar-reviews-carousel-header">
          <h4 class="sidebar-reviews-carousel-title">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="#4285F4" style="display:inline-block;"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" fill="#EA4335"/></svg>
            Google Reviews
          </h4>
          <span class="sidebar-reviews-google-badge">
            <span style="color:#f59e0b;">★</span> 5.0 (14)
          </span>
        </div>
        <div class="sidebar-reviews-viewport">
          <div class="sidebar-reviews-vertical-track">
            ${reviews.map((r, i) => `
              <div class="sidebar-review-slide" data-slide-index="${i}">
                <div class="sidebar-review-author">${r.author_name}</div>
                <div class="sidebar-review-stars">★★★★★</div>
                <p class="sidebar-review-text">"${r.text}"</p>
                <div class="sidebar-review-footer">
                  <span>Verified Google Review</span>
                  <span>${r.relative_time_description || 'Recent'}</span>
                </div>
              </div>
            `).join('')}
          </div>
        </div>
        <div class="sidebar-reviews-timer-bar"></div>
      `;

      container.classList.add('sidebar-reviews-carousel');

      const track = container.querySelector('.sidebar-reviews-vertical-track');
      const timerBar = container.querySelector('.sidebar-reviews-timer-bar');
      const slides = container.querySelectorAll('.sidebar-review-slide');
      let currentIndex = 0;
      let isPaused = false;
      let progress = 0;
      const slideHeight = 155;
      ...
      // 15-second progress ticker
      const timerInterval = setInterval(function () {
        if (isPaused) return;
        progress += (tickIntervalMs / cycleDurationMs) * 100;
        if (timerBar) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initSidebarReviewsCarousels);
  } else {
