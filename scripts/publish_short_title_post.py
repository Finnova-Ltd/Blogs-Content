...
ts = int(time.time())
slug = f"procrm-ai-governance-{ts}"
title = f"PRO CRM Agentforce AI Governance Playbook ({ts})"
...
os.system(f'cd "{PROCRM_DIR}" && git commit -am "Publish short title post {slug}" && git push origin main')
print(f"🚀 Published short title post: {title}")
