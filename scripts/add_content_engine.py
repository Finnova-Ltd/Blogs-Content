...
content_engine_py = '''#!/usr/bin/env python3
"""
...
class MultiSourceContentEngine:
    ...
    def publish_project(self, project_cfg):
        ...
        for item in candidates[:project_cfg.get("max_publish_per_run", 4)]:
            ...
            # Write HTML if html_dir is configured
            if output_html_dir:
            ...
            # Dispatch to Make.com webhook if configured
            if make_webhook:
...
# Example multi-project config supporting keywords like home buyers, centrelink, mortgage
config_example_json = '''{
  "projects": [
    {
      ...
      "sources": [
        ...
        {
          "type": "rss",
          "value": "https://www.google.com/alerts/feeds/14625353401416373956/18413967573759855438",
          "category": "Home Loans"
        },
        {
          "type": "url",
          "value": "https://www.rba.gov.au/media-releases/",
          "category": "RBA & Monetary Policy"
        }
      ],
      "max_publish_per_run": 4,
      "posts_json_path": "output/ezmortgagebroker_posts.json",
      "html_dir": "output/pages/blog",
      "make_webhook_url": ""
    }
  ]
}
...
# Write to repo
with open(os.path.join(REPO_DIR, "content_engine.py"), "w", encoding="utf-8") as f:
    f.write(content_engine_py)

with open(os.path.join(REPO_DIR, "projects_config.example.json"), "w", encoding="utf-8") as f:
    f.write(config_example_json)

# Update README in repo with the new Content Engine section
readme_update = '''
---

## 🤖 Multi-Source Content Engine & Blog Rewriter (`content_engine.py`)

For each project (e.g. `ezmortgagebroker.com.au`, `procrm.com.au`, etc.), you can configure **multiple sources** and **keyword filters** (e.g. `home buyers`, `centrelink`, `mortgage`, `rates`). The engine automatically:
1. Polls all sources (Yahoo Finance, Google Alerts, RSS, Webpages, WordPress).
2. Filters strictly by your keywords.
3. Rewrites the raw news into high-SEO 180–200 word value-dense blog articles in your brand voice.
4. Generates JSON-LD schema & HTML pages.
5. Dispatches formatted social payloads to **Make.com** for automatic posting to Facebook and LinkedIn!

### Quick Run:
```bash
python3 content_engine.py projects_config.example.json
```

### Sample Configuration (`projects_config.json`):
```json
{
  "projects": [
    {
      ...
      "brand_url": "https://ezmortgagebroker.com.au",
      "author": "R BAKSHI",
      "keywords": ["home buyers", "centrelink", "mortgage", "interest rates", "first home"],
      "sources": [
        { "type": "yahoo_finance", "value": "CBA.AX", "category": "Banking & Rates" },
        { "type": "rss", "value": "https://www.google.com/alerts/feeds/.../...", "category": "Home Loans" },
        { "type": "url", "value": "https://www.rba.gov.au/media-releases/", "category": "RBA & Rates" }
      ],
...
print("✅ Added content_engine.py and projects_config.example.json to Finnova-Ltd/rss!")
